...
assert.equal(multiple.issues[0].category,'action_required')
const noBilling=legacyScheduleSetupReview([{...row,contract:{...contract,billing_method:null,annual_maintenance_inc:null,billing_amount_overrides:null}}],recipients,[],'2026-09-20')
assert.equal(noBilling.issues[0].category,'no_billing_candidate')
assert.equal(noBilling.issues[0].code,'method_missing_without_amount')
const missingMethod=legacyScheduleSetupReview([{...row,contract:{...contract,billing_method:null}}],recipients,[],'2026-09-20')
assert.equal(missingMethod.issues[0].category,'action_required')
assert.equal(missingMethod.issues[0].code,'method_missing_with_amount')
