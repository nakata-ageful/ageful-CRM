...
# 最新：請求＋保守の全件照合、顧客履歴の純粋取得準備

reconcile-billing-import.cjsはmaintenance_only保全証跡を正式に照合する。元全JSON/hash/親の一致に加え、空unit_ids/空payloads/請求回なしを要求。入金予定日等を含む曖昧行は保守扱いせず拒否。請求と保守が混在するbackup、および保守だけのbackupに対応。保守メモ変更・不正控え・請求証跡の保守偽装は不一致。全件一致でもcutoverReady=false。照合用JSON内の証跡検証でありDB操作完了/権限/復元の証明ではない。

billing-history.tsは認可済み回別データを保存請求先で横断抽出。現project.customer_idは使わない。過去Aと今後Bを分離し、履歴の金額不明は0円として集計せず件数を別表示、予定/要確認/取りやめも別計数。重複IDを拒否し値コピー。通常画面のDB横断取得・認可接続は未完で、この純粋関数だけで旧所有者の画面が完成したとはしない。
