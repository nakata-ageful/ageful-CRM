-- Follow-up draft for the isolated new DB. The old DB is never a target.
-- One explicitly confirmed, previously unsaved occurrence can be created in
-- the same transaction as transfer. Existing 10-argument RPC remains stable.
...
CREATE OR REPLACE FUNCTION public.transfer_ownership_manual(p_key uuid,p_project bigint,p_owner bigint,p_date date,
  p_expected_project jsonb,p_expected_contract jsonb,p_fields jsonb,p_choices jsonb,p_reason text,p_future_recipient bigint DEFAULT NULL)
...
DECLARE actor uuid:=auth.uid(); op public.billing_operations%ROWTYPE; old_project public.projects%ROWTYPE;
  ...
  patches jsonb; project_patch jsonb; contract_patch jsonb; assignments text; fingerprint text; child uuid; next_child uuid;
  old_name text; transfer_id bigint; c jsonb; existing_choices jsonb:='[]'::jsonb; next_item jsonb;
  versions jsonb; last_management bigint;
BEGIN
  ...
  fingerprint:=encode(sha256(convert_to(jsonb_build_object('action','manual_transfer','project',p_project,'owner',p_owner,
    ...
    'choices',p_choices,'reason',p_reason,'future_recipient',coalesce(p_future_recipient,p_owner),'actor',actor)::text,'UTF8')),'hex');
  ...
  IF coalesce(p_future_recipient,p_owner) NOT IN(old_project.customer_id,p_owner) THEN RAISE EXCEPTION 'その後の基本の請求先を確認してください'; END IF;
  ...
  IF EXISTS(SELECT 1 FROM public.annual_records WHERE contract_id=old_contract.id)
    AND NOT EXISTS(SELECT 1 FROM public.billing_migration_acceptances WHERE project_id=p_project) THEN
    RAISE EXCEPTION '旧年度記録の移行・照合が完了していません'; END IF;
  FOR c IN SELECT value FROM jsonb_array_elements(p_choices) LOOP
    IF c ? 'newOccurrence' THEN
      IF next_item IS NOT NULL OR (SELECT count(*) FROM jsonb_object_keys(c))<>1
        OR jsonb_typeof(c->'newOccurrence') IS DISTINCT FROM 'object' THEN RAISE EXCEPTION '未保存の回は1件ずつ確認してください'; END IF;
      next_item:=c->'newOccurrence';
      IF (next_item->>'recipientId')::bigint NOT IN(old_project.customer_id,p_owner) THEN
        RAISE EXCEPTION '未保存回の請求先は変更前後の所有者から選んでください'; END IF;
    ELSE
      IF (c->>'recipientId')::bigint NOT IN(old_project.customer_id,p_owner)
        AND NOT EXISTS(SELECT 1 FROM public.billing_units u WHERE u.project_id=p_project AND u.id=(c->>'unitId')::bigint
          AND u.recipient_customer_id=(c->>'recipientId')::bigint) THEN
        RAISE EXCEPTION '請求先は変更前後の所有者、またはその回の元の請求先を指定してください'; END IF;
      existing_choices:=existing_choices||jsonb_build_array(c);
    END IF;
  END LOOP;
  ...
  INSERT INTO public.billing_recipient_plans(project_id,default_recipient_customer_id,effective_from)
    VALUES(p_project,coalesce(p_future_recipient,p_owner),p_date);
  child:=substr(encode(sha256(convert_to(p_key::text||':manual-plan','UTF8')),'hex'),1,32)::uuid;
  PERFORM public.write_manual_billing_plan(child,p_project,existing_choices,p_reason);
  ...
  IF next_item IS NOT NULL THEN
    ...
    SELECT coalesce(max(id),0) INTO last_management FROM public.project_management_events WHERE project_id=p_project;
    next_child:=substr(encode(sha256(convert_to(p_key::text||':next-occurrence','UTF8')),'hex'),1,32)::uuid;
    PERFORM public.create_future_schedule(next_child,p_project,to_jsonb(new_contract),versions,last_management,jsonb_build_array(next_item),p_reason);
  END IF;
  INSERT INTO public.ownership_transfers(operation_key,project_id,from_customer_id,to_customer_id,transfer_date,
    ...
    VALUES(p_key,p_project,old_project.customer_id,p_owner,p_date,to_jsonb(old_contract),to_jsonb(new_contract),to_jsonb(old_project),to_jsonb(new_project),
      jsonb_build_object('default','keep','choices',p_fields,'billing_choices',existing_choices,'next_occurrence',next_item,'reason',p_reason,'future_recipient',coalesce(p_future_recipient,p_owner)),
      jsonb_build_object('scope','manual_transfer','billing_operation',child,'next_operation',next_child,'migration_accepted',EXISTS(SELECT 1 FROM public.billing_migration_acceptances WHERE project_id=p_project)),actor) RETURNING id INTO transfer_id;
...
END $$;
REVOKE ALL ON FUNCTION public.transfer_ownership_manual(uuid,bigint,bigint,date,jsonb,jsonb,jsonb,jsonb,text,bigint) FROM PUBLIC;
