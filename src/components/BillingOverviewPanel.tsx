...
export function BillingOverviewPanel({data,today}:{data:BillingHistoryData;today:string}) {
  ...
  return <section><h2>請求</h2>
    ...
    {([['今月・来月・再来月の請求予定',overview.upcoming],['未入金',overview.unpaid],['入金済',overview.received]] as const).map(([label,units])=>
      <div key={label}><h3>{label}（{units.length}件）</h3><BillingHistorySection data={{...data,units}} /></div>)}
