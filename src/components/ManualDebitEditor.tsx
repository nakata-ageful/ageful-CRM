...
export function ManualDebitEditor({unit:initialUnit,recipientName,onSave,onClose}:{unit:BillingUnit;recipientName:(id:number)=>string;
  ...
  const [mode,setMode]=useState<'debit_received'|'debit_invoice_switch'>('debit_received')
  ...
  if(unit.method!=='口座振替'||unit.lifecycle!=='planned'||unit.frozenAmount!==null||unit.receivedOn)return <p role="alert">未確認の振替予定だけが対象です。</p>
  async function submit(e:React.FormEvent){
    ...
    try{
      ...
      let value:object={}
      if(mode==='debit_received'){
        if(!isBillingDate(date)||items.some(i=>!i.name.trim()||!/^\d+$/.test(i.amount)||!Number.isSafeInteger(Number(i.amount))))throw Error('入金日・実額・明細を入力してください')
  ...
  return <section className="card"><h3>{unit.serviceYear}年 {unit.roundLabel}：振替結果を記録</h3>
    ...
    {error&&<p role="alert">{error}</p>}<form onSubmit={submit}><fieldset disabled={busy} style={{border:0}}>
      <label>確認結果<select value={mode} onChange={e=>setMode(e.target.value as typeof mode)}><option value="debit_received">入金を確認した</option><option value="debit_invoice_switch">振替不能を確認し、請求書へ切替</option></select></label>
      {mode==='debit_received'?<><label>入金日<input required type="date" value={date} onChange={e=>setDate(e.target.value)}/></label>
