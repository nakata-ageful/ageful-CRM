...
try {
  ...
  await db.transaction(async tx => {
    ...
    await tx.exec(sql('20260907_manual_debit_result.sql'))
  })
  ...
  const debitIds=(await db.query("insert into billing_units(project_id,contract_id,occurrence_key,service_year,service_month,original_method,collection_method,recipient_customer_id,recipient_source,planned_amount) values(1,1,'debit-jun',2026,6,'direct_debit','direct_debit',1,'confirmed',82500),(1,1,'debit-jul',2026,7,'direct_debit','direct_debit',1,'confirmed',82500) returning id")).rows.map(r=>r.id)
  const debit=(op,id,revision,kind,payload,reason='銀行明細を手動確認')=>db.query('select record_manual_debit_result($1,$2,$3,$4,$5::jsonb,$6)',[op,id,revision,kind,JSON.stringify(payload),reason])
  const actual={received_on:'2026-06-27',amount:82500,line_items:[{name:'保守料',amount:82500}]}
  const debitBefore=await row(debitIds[0]),debitCounts=await counts()
  await assert.rejects(debit(key(),debitIds[0],0,'received',{...actual,amount:1}),/total do not match/)
  assert.deepEqual(await row(debitIds[0]),debitBefore)
  assert.deepEqual(await counts(),debitCounts)
  await assert.rejects(debit(key(),debitIds[0],0,'received',actual,''),/確認内容/)
  const debitOp=key()
  await db.exec('create trigger fail_debit_audit before insert on billing_unit_events for each row execute function test_fail_audit()')
  await assert.rejects(debit(key(),debitIds[0],0,'received',actual),/synthetic audit failure/)
  assert.deepEqual(await row(debitIds[0]),debitBefore)
  assert.deepEqual(await counts(),debitCounts)
  await db.exec('drop trigger fail_debit_audit on billing_unit_events')
  await debit(debitOp,debitIds[0],0,'received',actual)
  await debit(debitOp,debitIds[0],0,'received',actual)
  assert.equal((await row(debitIds[0])).revision,1)
  assert.equal((await row(debitIds[0])).recipient_customer_id,1)
  assert.equal((await row(debitIds[0])).planned_amount,82500)
  assert.equal((await row(debitIds[0])).frozen_amount,82500)
  assert.equal((await row(debitIds[0])).issued_on,null)
  await assert.rejects(debit(key(),debitIds[0],1,'invoice_switch',{}),/未確認/)
  await assert.rejects(debit(key(),debitIds[1],0,'invoice_switch',{recipient_customer_id:2}),/維持/)
  const switchOp=key()
  await debit(switchOp,debitIds[1],0,'invoice_switch',{},'振替不能を確認、請求書へ切替')
  await debit(switchOp,debitIds[1],0,'invoice_switch',{},'振替不能を確認、請求書へ切替')
  const switched=await row(debitIds[1])
  assert.equal(switched.original_method,'direct_debit')
  assert.equal(switched.collection_method,'invoice')
  assert.equal(switched.collection_state,'failed')
  assert.equal(switched.recipient_customer_id,1)
  assert.equal(switched.frozen_amount,null)
  await write(key(),debitIds[1],1,'issue',value())
  await write(key(),debitIds[1],2,'collection',{received_on:'2026-08-01'})
  assert.equal((await row(debitIds[1])).lifecycle,'received')
  assert.equal((await row(debitIds[1])).original_method,'direct_debit')
  console.log('PASS: manual debit receipt and same-unit invoice fallback, original payer/estimate/history, atomic validation and stable retry')
  const estimateId=(await db.query("insert into billing_units(project_id,contract_id,occurrence_key,service_year,original_method,collection_method) values(1,1,'saved-estimate',2026,'invoice','invoice') returning id")).rows[0].id
  const planValue={recipient_customer_id:1,scheduled_date:'2026-12-01'}
  const estimateOp=key()
  await write(estimateOp,estimateId,0,'plan',{...planValue,planned_amount:82500})
  assert.equal((await row(estimateId)).planned_amount,82500)
  assert.equal((await row(estimateId)).frozen_amount,null)
  await write(estimateOp,estimateId,0,'plan',{...planValue,planned_amount:82500})
  assert.equal((await row(estimateId)).revision,1)
  await write(key(),estimateId,1,'plan',planValue)
  assert.equal((await row(estimateId)).planned_amount,82500,'Omission preserves saved estimate')
  for(const amount of [-1,1.5,'82500',9007199254740992])
    await assert.rejects(write(key(),estimateId,2,'plan',{...planValue,planned_amount:amount}))
  assert.equal((await row(estimateId)).revision,2)
  await write(key(),estimateId,2,'plan',{...planValue,planned_amount:0})
  assert.equal((await row(estimateId)).planned_amount,0)
  await write(key(),estimateId,3,'plan',{...planValue,planned_amount:null})
  assert.equal((await row(estimateId)).planned_amount,null)
  await write(key(),estimateId,4,'plan',{...planValue,planned_amount:100})
  await write(key(),estimateId,5,'issue',value())
  assert.equal((await row(estimateId)).planned_amount,100,'Keep estimate history distinct from issued actual')
  assert.equal((await row(estimateId)).frozen_amount,82500)
  await assert.rejects(write(key(),estimateId,6,'plan',{...planValue,planned_amount:200}),/この状態/)
  const estimateEvent=(await db.query("select before_value,after_value from billing_unit_events where operation_key=$1",[estimateOp])).rows[0]
  assert.equal(estimateEvent.before_value.planned_amount,null)
  assert.equal(estimateEvent.after_value.planned_amount,82500)
  await db.exec('create role test_anon; set role test_anon')
  await assert.rejects(debit(key(),debitIds[0],1,'received',actual),/permission denied/)
