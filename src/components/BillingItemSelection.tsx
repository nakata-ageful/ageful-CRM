...
export function BillingItemSelection({contract,onSave}:{contract:Contract;onSave?:(flags:Record<string,boolean>)=>Promise<void>}){
  ...
  return <section className="card"><h3 className="section-title">費目ごとの自社請求対象</h3>
    ...
    <fieldset disabled={busy||!onSave} style={{border:0,padding:0}}>
      ...
      <p>自社請求対象額（費目の年間合計）：{fmtYen(summary.includedTotal)} ／ 対象外の記録額：{fmtYen(summary.excludedTotal)}</p>
