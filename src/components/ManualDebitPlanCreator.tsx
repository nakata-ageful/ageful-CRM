...
export function ManualDebitPlanCreator({onSave}:{onSave:(value:NewDebitPlan)=>Promise<void>}){
 const [recipient,setRecipient]=useState('2'),[year,setYear]=useState(''),[month,setMonth]=useState(''),[date,setDate]=useState(''),[amount,setAmount]=useState(''),[note,setNote]=useState(''),[reason,setReason]=useState('')
 ...
 async function save(e:React.FormEvent){e.preventDefault();if(lock.current)return;lock.current=true;setBusy(true);setError('')
  try{if(!/^\d{4}$/.test(year)||Number(year)<2000||Number(year)>2200||!/^\d{1,2}$/.test(month)||Number(month)<1||Number(month)>12||!isBillingDate(date)||!reason.trim()
 ...
 <label>請求先<select value={recipient} onChange={e=>setRecipient(e.target.value)}><option value="1">顧客A</option><option value="2">顧客B</option></select></label>
 ...
 <button type="submit" className="btn btn-main">{busy?'保存中…':'検証用DBへ追加'}</button><p role="status">{error}</p>
