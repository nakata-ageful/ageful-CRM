...
BEGIN
  ...
  INSERT INTO public.ownership_transfers(operation_key,project_id,from_customer_id,to_customer_id,transfer_date,
    ...
    VALUES(p_key,p_project,old_project.customer_id,p_owner,p_date,to_jsonb(old_contract),to_jsonb(new_contract),to_jsonb(old_project),to_jsonb(new_project),
      jsonb_build_object('default','keep','choices',p_fields,'billing_choices',p_choices,'reason',p_reason,'future_recipient',coalesce(p_future_recipient,p_owner)),
      jsonb_build_object('scope','manual_transfer','billing_operation',child,'migration_accepted',EXISTS(SELECT 1 FROM public.billing_migration_acceptances WHERE project_id=p_project)),actor) RETURNING id INTO transfer_id;
